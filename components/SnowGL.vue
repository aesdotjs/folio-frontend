...
onMounted(() => {
  ...
  const { count, windChange, minOpacity, depth, size: particleSize, gravity } = props;
  let wind = {
    current: 0,
    force: 0.1,
    target: 0.05,
  ...
  snow.value = new ShaderProgram(holder.value, canvas.value, {
    mousemove: false,
    mouseclick: false,
    depthTest: true,
    antialias: true,
  ...
  setTimeout(() => snow.value.resize(), 1000);
});
</script>
