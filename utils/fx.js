...
Ob.prototype.update = function (affectVector) {
  this.x =
    this.direction * affectVector.x * this.radius * affectVector.percentage;
  this.y =
    this.direction * affectVector.y * this.radius * affectVector.percentage;
...
FxWrap.prototype._getObjects = function () {
  ...
  self.objects = [...self.el.querySelectorAll(".js-object-wrap")].map(function (
    item
  ) {
...
SelfMoveOb.prototype.update = function (process) {
  ...
  if (this.path == "fix") {
    unitVector = {
      x: process * this.unitVector.x * this.direction,
      y: process * this.unitVector.y * this.direction,
    };
  }

  self.x = unitVector.x * self.radius;
  self.y = unitVector.y * self.radius;
  self.el.style.WebkitTransform = self.el.style.transform = `
		translate(${self.x}px, ${self.y}px)
	`;
};

// seflMove object manager, combine selfMove Object to 1 group
let SelfMoveObManager = function ({ duration, TweenMax }) {
  this.obs = [];
...
SelfMoveObManager.prototype.startProcess = function () {
  ...
  self._progressTween && self._progressTween.kill(),
    ...
    (self._progressTween = self.TweenMax.to(self, self.duration, {
      _progress: 1,
      ease: "none",
      onUpdate: function () {
      ...
      onComplete: function () {
        self.TweenMax.delayedCall(0 * Math.random(), function () {
          (self._progress = 0), self.startProcess();
          self.obs.map((ob) => {
            ob.updateDirection();
            // console.log(ob)
          });
...
let ObGroupManager = function (TweenMax) {
  this.groups = [];
  this.TweenMax = TweenMax;
  this._init();
  this._run();
...
module.exports = { ObGroupManager, FxWrap };
